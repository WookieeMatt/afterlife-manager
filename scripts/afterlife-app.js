...
export class AfterlifeDashboard extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static DEFAULT_OPTIONS = {
        id: "afterlife-dashboard", title: "AFTERLIFE OS v2.5", tag: "form",
        classes: ["afterlife-window"], window: { resizable: true, width: 650, height: 800 },
        actions: {
    ...
    async _prepareContext() {
        ...
        const mapData = (r) => {
            const reqUser = game.users.get(r.requestedBy);
            const displayName = (reqUser && reqUser.isGM) ? designatedFixerName : (reqUser?.name || "Unknown");
